...
{
	cout << "USAGE : ./multi-openpose <VideoFile> " << endl;

	string videoFile = "C:\\ConsoleApplication1\\dro.avi";
	...
	cv::dnn::Net inputNet = cv::dnn::readNetFromCaffe("C:\\ConsoleApplication1\\pose\\coco\\pose_deploy_linevec.prototxt", "C:\\ConsoleApplication1\\pose\\coco\\pose_iter_440000.caffemodel");
	...
	while (cv::waitKey(1) < 0)
	{
		...
		cv::Mat inpBlob = cv::dnn::blobFromImage(frame, 1.0 / 255.0, cv::Size((int)((368 * frameCopy.cols) / frameCopy.rows), 368), cv::Scalar(0, 0, 0), false, false);
		inputNet.setInput(inpBlob);

		cv::Mat netOutputBlob = inputNet.forward();

		std::vector<cv::Mat> netOutputParts;
		splitNetOutputBlobToParts(netOutputBlob, cv::Size(frameCopy.cols, frameCopy.rows), netOutputParts);
		...
		double time = inputNet.getPerfProfile(layersTimes) / freq;
		double fps = 100 / time;

		cv::putText(outputFrame, cv::format("Nvidia RTX  %.2f FPS ", fps), cv::Point(50, 50), cv::FONT_HERSHEY_COMPLEX, .8, cv::Scalar(255, 50, 0), 2);
		// imshow("Output-Keypoints", frameCopy);
		imshow("Output-Skeleton", outputFrame);
